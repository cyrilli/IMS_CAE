...
class Conv2d(Layer):
    ...
    def __init__(self, kernel_size, strides, output_channels, name, is_training, initializer = None):
    ...
    def create_layer_reversed(self, input, prev_layer=None):
        ...
        with tf.variable_scope('conv', reuse=True):
            W = tf.get_variable('W{}'.format(self.name[-2:]))  # reuse W from conv
        ...
        output = lrelu(tf.add(utils.batch_norm_layer(output, self.is_training,'BN{}'.format(self.name[-2:])+'_reverse'), b))
        #output = lrelu(tf.add(tf.contrib.layers.batch_norm(output, decay=0.999, center=True, scale=True, updates_collections=None,is_training=True, reuse=None), b))
